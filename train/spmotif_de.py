...
from utils.measure import calc_acc, ood_detection, get_gpu_memory_map
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Training arguments for SPMotif')
    ...
    parser.add_argument('--bias', default='0.9', type=str, help='select inlier bias')
    parser.add_argument('--ood_bias', default='0.9', type=str, help='select outlier bias')
    # hyper
    ...
    parser.add_argument('--backbone', default='SAGPool', type=str, help='select backbone model')
    ...
    if args.graphde_v or args.graphde_a:
...
if __name__ == '__main__':
    ...
    # i.i.d. dataset
    train_dataset = SPMotif(osp.join(dataset_dir, f'SPMotif-{args.bias}/'), mode='train')
    val_dataset = SPMotif(osp.join(dataset_dir, f'SPMotif-{args.bias}/'), mode='val')
    in_test_dataset = SPMotif(osp.join(dataset_dir, f'SPMotif-{args.bias}/'), mode='test')
    # o.o.d. dataset
    mixed_dataset = SPMotif(osp.join(dataset_dir, f'SPMotif-mixed-{args.ood_bias}/'), mode='train')
    out_test_dataset = SPMotif(osp.join(dataset_dir, f'SPMotif-ood-{args.ood_bias}/'), mode='test')

    # shuffle the dataset if necessary
    # perm_idx = torch.randperm(len(train_dataset), generator=torch.Generator().manual_seed(0))
    # train_dataset = train_dataset[perm_idx]
    ...
    n_id_train = len(train_dataset)-n_mixed_train

    # we need to modify outliers' idx to track their gradients (for GraphDE-v)
    for data in mixed_dataset[:n_mixed_train]:
        data.idx += n_id_train
    ...
    n_train_data, n_val_data, n_in_test_data, n_out_test_data = len(train_dataset), len(val_dataset), len(in_test_dataset), len(out_test_dataset)
    ...
    experiment_name = f'spmotif.graphde-a_{args.graphde_a}.graphde-v_{args.graphde_v}.backbone_{args.backbone}.' \
                    f'id-bias{args.bias}.ood-bias{args.ood_bias}.ood-ratio_{args.biased_ratio}.prior-ratio{args.prior_ratio}.' \
    ...
    for seed in args.seed:
        ...
        g = SPMotifNet(in_channels, num_classes, n_train_data=n_train_data, args=args).to(device)
        ...
        with torch.no_grad():
            ...
            logger.info("Seed {:d} Base AUROC:{:.3f} Base AUPR:{:.3f} Base FPR95:{:.3f} "
                ...
                seed, 100 * base_auroc, 100 * base_aupr, 100 * base_fpr, 100 * vi_auroc, 100 * vi_aupr, 100 * vi_fpr))
    ...
    logger.info("Train ACC:{:.4f}-+-{:.4f} Val ACC:{:.4f}-+-{:.4f} In Test ACC:{:.4f}-+-{:.4f} Out Test ACC:{:.4f}-+-{:.4f} "
